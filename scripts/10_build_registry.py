...
# Native Word extraction is preferred when available.
# PDF is still selected when no Word version exists.
EXT_PRIORITY = {
    ".docx": 1,
    ".doc": 2,
    ".pdf": 3,
...
def select_group_variants(items: List[Dict[str, Any]]):
    ...
    Selection rule:
      ...
      - if no Word exists, select PDF
      - known languages: select one best file per ru/uz_cyr/uz_lat
      - unknown language: keep one best per normalized_file_key
    """
    ...
    for lang in ["ru", "uz_cyr", "uz_lat"]:
        ...
        best = dict(choose_best(lang_items))
        best["selected"] = True
    ...
    for _, subitems in unknown_groups.items():
        best = dict(choose_best(subitems))
        best["selected"] = True
